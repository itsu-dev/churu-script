...
export type BlockWithArgumentsStmt = {
	arguments: string[];
	statements: Stmt[];
} & Stmt;
...
export type VarStmt = {
	name: Token;
	initializer: Expr;
	withStatements?: BlockWithArgumentsStmt;
} & Stmt;
...
export default function Parser(tokens: Array<Token>): ReturnType {
	...
	const attribute = (): [string, Expr] => {
		...
		if (match(TOKEN_TYPE_STRING)) {
			return [
				`${key.lexeme}`,
				{ value: previous().lexeme, type: TOKEN_TYPE_STRING } as LiteralExpr,
			];
		}
		...
		return [
			`${key.lexeme}`,
			{ value: null, type: TOKEN_TYPE_KEYWORD_NULL } as LiteralExpr,
		];
	};
	...
	const primary = (): Expr => {
		...
		if (
			...
			return {
				value:
					prev.type === TOKEN_TYPE_NUMBER
						? parseFloat(prev.lexeme!)
						: prev.lexeme,
				type: prev.type,
			} as LiteralExpr;
		}
	...
	const args = (): string[] => {
		...
		while (!check(TOKEN_TYPE_RIGHT_PAREN) && !isAtEnd()) {
			match(TOKEN_TYPE_IDENTIFIER);
			args.push(previous().lexeme!);
			match(TOKEN_TYPE_COMMA);
		}
	...
	const blockWithArgument = (): Stmt => {
		if (!match(TOKEN_TYPE_LEFT_BRACE)) {
			throw new Error('Expect "{" before block statements');
		}
		...
		if (match(TOKEN_TYPE_LEFT_PAREN)) {
			...
			if (!match(TOKEN_TYPE_RIGHT_PAREN)) {
				throw new Error('Expect ")" after block arguments');
			}

			if (!match(TOKEN_TYPE_ARROW)) {
				throw new Error('Expect "=>" after block arguments');
			}
		}
		...
		while (!match(TOKEN_TYPE_RIGHT_BRACE) && !isAtEnd()) {
			stmts.push(statement());
		}

		return { arguments: args0, statements: stmts } as BlockWithArgumentsStmt;
	};
	...
	const varDecl = (): Stmt => {
		...
		let withStatement: BlockWithArgumentsStmt | undefined = undefined;

		if (match(TOKEN_TYPE_ASSIGN_OPERATOR)) {
			initializer = expression();

			if (match(TOKEN_TYPE_KEYWORD_WITH)) {
				withStatement = blockWithArgument() as BlockWithArgumentsStmt;
			}
		}

		if (!match(TOKEN_TYPE_SEMICOLON)) {
			throw new Error('Expect ";" after variable declaration');
		}

		return { name, initializer, withStatement } as VarStmt;
	};

	const statement = (): Stmt => {
		switch (true) {
			case match(TOKEN_TYPE_KEYWORD_VAR): {
				return varDecl();
			}
		}

		throw new Error("Expect statement");
	};

	const program = (): Stmt[] => {
		const statements: Array<Stmt> = [];

		while (!match(TOKEN_TYPE_EOF)) {
